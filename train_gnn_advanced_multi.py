...
def train_for_locale(args):
    locale, all_locale_data, device_id = args
    device = torch.device(f'cuda:{device_id}')
    print(f"Process {os.getpid()} assigned to {device} for locale {locale}")

    print(f"\n===== Processing {locale} on {device} =====")
    ...
    print(f"Data prep for {locale}: {time.time() - timer:.2f} seconds | Nodes: {num_nodes}, Edges: {pyg_data.num_edges}")
    ...
    for epoch in range(1, gnn_epochs + 1):
        ...
        if patience >= max_patience:
            print(f"Early stopping at epoch {epoch} for {locale}...")
            break
    print(f"Trained {locale} in {time.time() - timer:.2f} seconds.")

    plot_and_save_history(history, locale, scheduler_name)
    model.load_state_dict(torch.load(model_save_path, map_location=device, weights_only=True))
    test_auc = test(model, test_data)
    print(f"Final AUC for {locale}: Validation: {best_val_auc:.4f}, Test: {test_auc:.4f}")
...
def main():
    ...
    try:
        ...
        print("Set start method to 'spawn'.")
    except RuntimeError:
        pass

    num_gpus = torch.cuda.device_count()
    if num_gpus == 0:
        print("No GPUs found. Running on CPU sequentially.")
        device = torch.device('cpu')
        all_locale_data = prepare_locale_artefacts()
        for i, locale in enumerate(LOCALES):
             train_for_locale((locale, all_locale_data, 'cpu')) # 'cpu' is not a valid device_id, needs adjustment: TO DO
        return

    print(f"Found {num_gpus} GPUs. Starting parallel training.")
    all_locale_data = prepare_locale_artefacts()

    args_list = [(locale, all_locale_data, i % num_gpus) for i, locale in enumerate(LOCALES)]

    with mp.Pool(processes=num_gpus) as pool:
        pool.map(train_for_locale, args_list)
