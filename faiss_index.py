...
def create_locale_indices(locale, locale_embeddings, index_type, hyperparameters, index_files, batch_size):
    if os.path.exists(index_files[locale]):
        ...
        print(f"FAISS {retrieved_type} Index for locale {locale} found at {index_files[locale]} | Total vectors: {index.ntotal} | Is trained: {index.is_trained}")
    else:
        ...
        print(f"FAISS {index_type} Index for locale {locale} created at {index_files[locale]} | Total vectors: {index.ntotal} | Is trained: {index.is_trained}")
    return index