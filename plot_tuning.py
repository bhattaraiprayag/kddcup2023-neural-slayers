...
plt.grid(True)
plt.savefig('1. best_val_auc_vs_training_time.png')
plt.close()
...
plt.ylabel('Test AUC')
plt.grid(True)
plt.savefig('2. test_auc_vs_best_val_auc.png')
plt.close()
...
plt.grid(axis='y')
plt.tight_layout()
plt.savefig('3. best_val_auc_by_scheduler.png')
plt.close()
...
plt.grid(axis='y')
plt.tight_layout()
plt.savefig('4. test_auc_by_scheduler.png')
plt.close()
...
plt.savefig('5. pair_plot_numerical_vars.png')
plt.close()
...
plt.grid(axis='y')
plt.savefig('6. avg_best_val_auc_by_locale.png')
plt.close()
...
plt.grid(axis='y')
plt.savefig('7. avg_test_auc_by_locale.png')
plt.close()
...
plt.tight_layout()
plt.savefig('8. correlation_heatmap.png')
plt.close()
...
plt.tight_layout()
plt.savefig('9. best_val_auc_vs_lr_by_scheduler.png')
plt.close()
...
plt.legend(title='Scheduler', bbox_to_anchor=(1.05, 1), loc='upper left')
plt.tight_layout()
plt.savefig('10. best_val_auc_vs_weight_decay_by_scheduler.png')
plt.close()

# Visualization 1: Faceted Scatter Plots (Corrected)
# Create a plot with subplots for each scheduler using relplot
g = sns.relplot(
    data=df_selected,
    x='lr',
    y='best_val_auc',
...
g.set(xscale="log") # Use a log scale for learning rate
g.fig.tight_layout()

# Save the figure
plt.savefig('11. revamped_1_auc_vs_lr_by_scheduler.png', dpi=300)
plt.close()

# Visualization 2: Hyperparameter Interaction Heatmap
# Note: This works best for grid search, but pivoting can approximate for random search.
# We will create a pivot table to aggregate the results.
...
# Save the figure
plt.savefig('12. revamped_2_heatmap_lr_vs_wd.png', dpi=300)
plt.close()

# Visualization 3: Multi-variable Scatter Plot
plt.figure(figsize=(12, 8))
scatter = plt.scatter(
...
# Create a colorbar and a legend for size
cbar = plt.colorbar(scatter)
cbar.set_label('Best Validation AUC', rotation=270, labelpad=15)

# Create a legend for the point sizes
...
# Save the figure
plt.savefig('13. revamped_3_wd_vs_hidden_channels.png', dpi=300)
plt.close()
...
# Save the figure
plt.savefig('14. revamped_4_violin_scheduler_vs_hidden.png', dpi=300)
plt.close()

# For this advanced visualization, we'll use the plotly library,
# which creates interactive HTML plots.
# You may need to install it: pip install pandas plotly
...
# For categorical data like scheduler, we first convert it to a numerical representation
df_parallel['scheduler_id'] = pd.Categorical(df_parallel['scheduler']).codes

# Add scheduler_id to the list of columns to plot
...
# Replace the numerical scheduler IDs with the actual names in the tick labels
fig.update_layout(
    xaxis=dict(
...
fig.write_html("15. revamped_5_parallel_coordinates.html")

# To display in a notebook, you would just call:
# fig.show()