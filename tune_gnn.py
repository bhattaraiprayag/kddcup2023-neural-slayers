...
def plot_and_save_history(history, locale, trial_id):
    fig, (ax1, ax2) = plt.subplots(2, 1, figsize=(12, 10), sharex=True)
    epochs = range(1, len(history['train_loss']) + 1)
    ax1.plot(epochs, history['train_loss'], 'bo-', label='Train Loss')
    ax1.set_ylabel('BCE Loss')
    ax1.set_title(f'Training Metrics for {locale} (Trial: {trial_id})')
    ax1.legend()
    ax1.grid(True)
    ...
    ax2.grid(True)
    plt.tight_layout()
    plot_save_path = os.path.join("outputs", "tuning", "plots", f'history_{trial_id}.png')
...
def run_trial(locale, params, data, device):
    ...
    plot_path = plot_and_save_history(history, locale, trial_id)
